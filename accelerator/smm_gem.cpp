...
uint64_t smmStream_3Dnano(uint64_t rn, uint64_t out_cnt) {
    uint64_t res;
    ...
    "MOV %[output], X10;"
    : [output] "=r"(res)
    : [input_k] "r"(rn), [input_i] "r"(out_cnt)
    : "x7", "x8", "x10"
    );
...
* -- rn = Parameter value.
*/
uint64_t smmQueue_3Dnano(uint64_t rm, uint64_t rn, uint64_t out_cnt) {
    uint64_t res;


    // printf("ENQUEUE foo %u\n", rn);
    ...
    "MOV %[output], X10;"
    : [output] "=r"(res)
    : [input_j] "r"(rm), [input_i] "r"(out_cnt), [input_k] "r"(rn)
    : "x7", "x8", "x9", "x10"
    );
// 46481D2A
    // ".long 0x21089D2A;"
...
* -- rn = Parameter value.
 */
uint64_t smmParamWrite_3Dnano(uint64_t rm, uint64_t rn, int layer) {
    uint64_t res;

    // printf("LOADING FOR layer = %d\n", layer);
    ...
    "MOV %[output], X10;"
    : [output] "=r"(res)
    : [input_j] "r"(rm), [input_i] "r"(layer), [input_k] "r"(rn)
    : "x7", "x8", "x9", "x10"
    );
...
SystolicMatrixMultiplication_3D smm3D = SystolicMatrixMultiplication_3D();

bool smmParamWrite_3Dnano(int rm, uint32_t ra, int layer) {
    // printf("LOADING WEIGT : layer %d  | rm %d  | ra %d\n", layer, rm, ra);
    return smm3D.loadWeights(rm, ra, layer);
}

uint32_t smmQueue_3Dnano(int rm, uint32_t ra, int out_cnt) {

    return smm3D.inputQueue_3Dnano(rm, ra, out_cnt);
}

uint32_t smmStream_3Dnano(uint32_t rn, int out_cnt) {

    return smm3D.streamInOut_3Dnano(rn, out_cnt);
}
#endif
...
// for(int id=0; id<N_3D_LAYERS; id++){
    ...
    for (int l2Col_3D_block = 0; l2Col_3D_block < col_blocks_in_3DStack; l2Col_3D_block++){
        ...
        for (int l2Row_3D_block = 0; l2Row_3D_block < row_blocks_in_3DStack; l2Row_3D_block++){
            ...
            // Counts the input words inside a single input group
            // Needs to know when the queue function should return an input
            int in_cnt_group = 1;
            ...
            for (int i = 0; i < seq_len; i++) {
                for (int j = 0; j < (MAX_COL); j++) {
                    ...
                    // printf("\nIn cnt GRUP = %d\n", in_cnt_group);
                    // printf("In cnt % N3D = %d\n", in_cnt_group % N_3D_LAYERS);
                    
                    // std::cout << "IN = " << content << " --> ";
                    // int8_t *res_qk = (int8_t*)&content; 
                    // for(int j=0; j<4; j++){
                    //     printf("%d, ", res_qk[j]);
                    // }
                    // printf("\n");
                    ...
                    if (j == MAX_COL - 1) {
                        // printf("COMPUTE: \n");
                        mult = smmStream_3Dnano(content, out_cnt);
                        // printf("SA cycle = %d\n", sa_cycles);
                        in_cnt_group = 0;
